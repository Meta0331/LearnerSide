...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const categoriesSnapshot = await getDocs(collection(db, 'pecsCategories'));
        ...
        const uniqueCardCategories = [...new Set(cardsData.map(card => card.categoryId))];
        ...
        if (categoriesData.length > 0) {
          ...
          const firstCategoryCards = cardsData.filter(card => {
            ...
            console.log(`Card "${card.text}" category "${card.categoryId}" matches "${firstCategoryName}":`, matches);
            return matches;
          });
  ...
  const handleCategoryPress = (categoryId: string) => {
    ...
    const filteredCards = allCards.filter(card => {
      ...
      console.log(`Card "${card.text}" (category: "${card.categoryId}") matches "${selectedCategory.categoryName}":`, matches);
      return matches;
    });

    console.log('Filtered cards for category:', selectedCategory.categoryName, filteredCards);
    setDisplayedCards(filteredCards);

    // If no cards found, let's also try a case-insensitive match
    if (filteredCards.length === 0) {
      console.log('No exact matches, trying case-insensitive...');
      const caseInsensitiveFiltered = allCards.filter(card =>
        card.categoryId.toLowerCase() === selectedCategory.categoryName.toLowerCase()
      );
  ...
  const onHandlerStateChange = async (
    ...
    if (event.nativeEvent.state === 5) {
      ...
      const isInSentenceStrip = absoluteY >= sentenceStripTop &&
  ...
  const renderCard = ({ item, index }: { item: CardType; index: number }): JSX.Element => {
  ...
  const renderSentenceCard = (card: CardType, index: number): JSX.Element => (
    <TouchableOpacity
      ...
      onPress={() => {
        setSentenceCards((prev: CardType[]) => prev.filter((_, i: number) => i !== index));
      }}
    >
      <View style={styles.sentenceCardImageContainer}>
        <Image
          source={card.image ? { uri: card.image } : require('@/assets/images/User.png')}
  ...
  const getItemLayout = (data: CardType[] | null | undefined, index: number) => ({
    ...
    <ThemedView style={styles.container}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.firstContainer}>
          <View style={[styles.sentenceStrip, { width: Math.min(width * 0.95, 1200) }]}>
            <TouchableOpacity
              style={[
                styles.button,
            ...
            <View style={styles.sentence}>
              {sentenceCards.length === 0 ? (
                <Text style={styles.dropHint}>Drag cards here to build a sentence</Text>
              ) : (
                <FlatList
                  data={sentenceCards}
                  renderItem={({ item, index }) => renderSentenceCard(item, index)}
      ...
      <View style={styles.footer}>
        <FlatList
          ...
          renderItem={({ item, index }) => (
            ...
            >
              <Image
                source={item.image ? { uri: item.image } : require('@/assets/images/pecs1.png')}
